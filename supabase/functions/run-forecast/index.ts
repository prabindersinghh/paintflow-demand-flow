...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Delete old forecasts for these products/regions
    ...
    for (const product of products!) {
      for (const region of regions) {
        // Get last 14 days of sales for this product-region
        ...
        if (recentSales && recentSales.length >= 7) {
          ...
          const avgFirst = firstHalf.reduce((s, r) => s + r.quantity, 0) / firstHalf.length
          const avgSecond = secondHalf.reduce((s, r) => s + r.quantity, 0) / secondHalf.length
          if (avgFirst > 0) {
